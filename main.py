...
def from_pdf_path_to_df(pdf_path: str, to_csv=True, refresh=False) -> pd.DataFrame:
    ...
    raw_texts: List[str] = pdf_p.extract_text_with_present_persons(matching_first_presence_page=v.matching_first_presence_page)
    ...
    for index_page, raw_text in zip(pdf_p.nb_pages_with_presence_index, raw_texts):
        ...
        for period, text_per_period in raw_text_per_day_period.items():
            data: dict = FromTextToDF.extract_names_according_to_status_and_return_list(text_per_period)
            tem_df = FromTextToDF.generate_df_from_segmented_data(data)
            tem_df['raw_time'] = period
            tem_df['index_page'] = index_page
            # if "absent" in tem_df["status"].values:
            #     absence = tem_df["status"].value_counts()["absent"]
            #     logger.warning(f"absence {absence} {index_page=}")
...
if __name__ == '__main__':
    ...
    if use_only_short_pdf:
        ...
        logger.info(f"len of pdf_files_path : {len(pdf_files_path)}")
